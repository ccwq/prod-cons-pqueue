...
export class ProdConsPQueue {
   ...
   * 跟踪当前正在被消费函数处理的项目集合
   */
  private consumingItems: Set<any> = new Set();
  ...
  private notifyStateChange(): void {
    ...
    if (currentBlocked !== this.previousBlocked) {
      ...
      // this.buffer.length >= this.slotAmount

      console.log("333", this.buffer.length, this.slotAmount)
  ...
  private scheduleConsumption(): void {
    ...
    while (this.buffer.length > 0 && this.queue.size < this.queue.concurrency) {
      const data = this.buffer.shift();
      if (data !== undefined) {
        this.consumingItems.add(data);
        this.notifyStateChange(); // 缓冲区减少，状态更新
        ...
        this.queue.add(async () => {
          try {
            ...
            // 消费完成后清理
            this.consumingItems.delete(data);
  ...
  async waitForEmpty(): Promise<void> {
    while ((this.buffer.length > 0 || this.consumingItems.size > 0) && this.consuming && !this.destroyed) {
  ...
  async clear(): Promise<void> {
    this.buffer = [];
    this.consumingItems.clear(); // 同时清空正在消费的跟踪项
  ...
  getStats(): {
    ...
    return {
      ...
      /** 在 p-queue 中排队等待执行的任务数 */
      pendingJobs: this.queue.pending,
      /** 在 p-queue 中当前正在执行的任务数 */
      runningJobs: this.queue.size,
      /** p-queue 是否已暂停 */
      isPaused: this.queue.isPaused,
      /** 正在被消费函数处理的项目总数 (包括正在运行和已提交到 p-queue 的) */
      consumingItems: this.consumingItems.size,
      /** 配置的并发数 */
